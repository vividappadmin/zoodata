...
function mailchimpSF_delete_setup() {
	...
	// Sushi ~Kai~ modification
	delete_option('mc_use_sushi_placeholder');
	delete_option('mc_use_sushi_pretag');
	// End Sushi
}
...
function mailchimpSF_set_form_defaults($list_name = '') {
	...
	// Sushi ~Kai~ modification
	update_option('mc_use_sushi_placeholder','off');
	update_option('mc_use_sushi_pretag','on');
	// End Sushi
}
...
function mailchimpSF_save_general_form_settings() {
	...
	// Sushi ~Kai~ modification 
	if (isset($_POST['mc_use_sushi_placeholder'])){
		update_option('mc_use_sushi_placeholder', 'on');
		$msg = '<p class="success_msg">'.__('[Sushi] Use placeholder turned On!', 'mailchimp_i18n').'</p>';
		mailchimpSF_global_msg($msg);
	} else if (get_option('mc_use_javascript')!='off') {
		update_option('mc_use_sushi_placeholder', 'off');
		$msg = '<p class="success_msg">'.__('[Sushi] Use placeholder turned Off!', 'mailchimp_i18n').'</p>';
		mailchimpSF_global_msg($msg);
	}
	
	if (isset($_POST['mc_use_sushi_pretag'])){
		update_option('mc_use_sushi_pretag', 'on');
		$msg = '<p class="success_msg">'.__('[Sushi] Distribute validation errors turned On!', 'mailchimp_i18n').'</p>';
		mailchimpSF_global_msg($msg);
	} else if (get_option('mc_use_javascript')!='off') {
		update_option('mc_use_sushi_pretag', 'off');
		$msg = '<p class="success_msg">'.__('[Sushi] Distribute validation errors Off!', 'mailchimp_i18n').'</p>';
		mailchimpSF_global_msg($msg);
	}
	// End Sushi
...
<tr valign="top">
	<th scope="row"><?php esc_html_e('Use Placeholders?', 'mailchimp_i18n'); ?>:</th>
    <td><input name="mc_use_sushi_placeholder" type="checkbox" <?php checked(get_option('mc_use_sushi_placeholder'), 'on'); ?> id="mc_use_sushi_placeholder" class="code" />
    <em><label for="mc_use_sushi_placeholder"><?php esc_html_e('turning this on will remove labels on your form inputs and replace it with HTML5 placeholder attribute.', 'mailchimp_i18n'); ?></label></em>
    </td>
</tr>
<tr valign="top">
	<th scope="row"><?php esc_html_e('Distribute Validation Errors?', 'mailchimp_i18n'); ?>:</th>
    <td><input name="mc_use_sushi_pretag" type="checkbox" <?php checked(get_option('mc_use_sushi_pretag'), 'on'); ?> id="mc_use_sushi_pretag" class="code" />
    <em><label for="mc_use_sushi_pretag"><?php esc_html_e('turning this on will distribute validation errors to each of their designated fields instead of displaying in one SPAN above the form.', 'mailchimp_i18n'); ?></label></em>
    </td>
</tr>
</table>
</div>
...
function mailchimpSF_signup_submit() {
	...
	$sushi_use_pretag = get_option( 'mc_use_sushi_pretag' );
	...
	foreach($mv as $var) {
		...
		// Sushi ~Kai~		
		$pretag =  ( $sushi_use_pretag == 'on' ) ? ':'.$opt.':' : '';
		// note: added $pretag on $errs
		// End Sushi
			...
			if (!$retval) {
				switch($api->errorCode) {
					...
					case '250' :
						...
						$errs[] = sprintf(__("%sYou must fill in %s.", 'mailchimp_i18n'), '', esc_html($mv_tag_keys[$field]['name']));
						break;
					case '254' :
					...
					case '270' :
						...
						break;
					case '502' :						
						$errs[] = ( $sushi_use_pretag == 'on' ) ? __(":mc_mv_EMAIL:That email address is invalid", 'mailchimp_i18n').'.' : __("That email address is invalid", 'mailchimp_i18n').'.';
						break;
					default:
	...
	if (count($errs) > 0) {
		...
		$msg = ( $sushi_use_pretag == 'on' ) ? '<span class="mc_error_msg" style="display: none">' : '<span class="mc_error_msg">';
